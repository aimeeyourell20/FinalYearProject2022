...
public class Austria_Fragment extends Fragment {

    private View AustriaView;
    private RecyclerView austriaRecyclerView;
    private DatabaseReference austria;
    private Button SearchButton;
    private EditText SearchInputText1;


    public Austria_Fragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        AustriaView =  inflater.inflate(R.layout.fragment_austria_, container, false);

        austriaRecyclerView = (RecyclerView) AustriaView.findViewById(R.id.austriaRecyclerView);
        austriaRecyclerView.setLayoutManager(new LinearLayoutManager(getContext()));

        austria = FirebaseDatabase.getInstance().getReference().child("users");

        SearchButton = (Button) AustriaView.findViewById(R.id.search_people_friends_button1);
        SearchInputText1 = (EditText) AustriaView.findViewById(R.id.search_box_input);
    ...
    public void onStart(String searchBoxInput1) {
        ...
        Query searchPeopleAndFriendsQuery = austria.orderByChild("all1")
                .startAt(searchBoxInput1).endAt(searchBoxInput1 + "\uf8ff");
        FirebaseRecyclerAdapter<FindMentor, Austria_Fragment.FindMentorViewHolder> firebaseRecyclerAdapter =
                new FirebaseRecyclerAdapter<FindMentor, Austria_Fragment.FindMentorViewHolder>(
                        FindMentor.class,
                        R.layout.all_mentors_displayed,
                        Austria_Fragment.FindMentorViewHolder.class,
                        searchPeopleAndFriendsQuery

                ) {
                    @Override
                    protected void populateViewHolder(Austria_Fragment.FindMentorViewHolder findMentorViewHolder, FindMentor findMentor, int i) {
                        ...
                        if(!findMentor.getLocation().equalsIgnoreCase("Austria")){
                            findMentorViewHolder.itemView.setVisibility(View.GONE);
                            findMentorViewHolder.itemView.setLayoutParams(new RecyclerView.LayoutParams(0, 0));
                        }
                        else {
        ...
        austriaRecyclerView.setAdapter(firebaseRecyclerAdapter);
    }
