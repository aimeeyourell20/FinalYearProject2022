...
public class Australia_Fragment extends Fragment {
    ...
    private View AustraliaView;
    private RecyclerView australiaRecyclerView;
    private DatabaseReference australia;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        AustraliaView =  inflater.inflate(R.layout.fragment_australia_, container, false);

        australiaRecyclerView = (RecyclerView) AustraliaView.findViewById(R.id.australiaRecyclerView);
        australiaRecyclerView.setLayoutManager(new LinearLayoutManager(getContext()));

        australia = FirebaseDatabase.getInstance().getReference().child("users");

        SearchButton = (Button) AustraliaView.findViewById(R.id.search_people_friends_button1);
        SearchInputText1 = (EditText) AustraliaView.findViewById(R.id.search_box_input);
    ...
    public void onStart(String searchBoxInput1) {
        ...
        Query searchPeopleAndFriendsQuery = australia.orderByChild("all1")
                .startAt(searchBoxInput1).endAt(searchBoxInput1 + "\uf8ff");
        FirebaseRecyclerAdapter<FindMentor, Australia_Fragment.FindMentorViewHolder> firebaseRecyclerAdapter =
                new FirebaseRecyclerAdapter<FindMentor, Australia_Fragment.FindMentorViewHolder>(
                        FindMentor.class,
                        R.layout.all_mentors_displayed,
                        Australia_Fragment.FindMentorViewHolder.class,
                        searchPeopleAndFriendsQuery

                ) {
                    @Override
                    protected void populateViewHolder(Australia_Fragment.FindMentorViewHolder findMentorViewHolder, FindMentor findMentor, int i) {
                        ...
                        if(!findMentor.getLocation().equalsIgnoreCase("Australia")){
                            findMentorViewHolder.itemView.setVisibility(View.GONE);
                            findMentorViewHolder.itemView.setLayoutParams(new RecyclerView.LayoutParams(0, 0));
                        }
                        else {
        ...
        australiaRecyclerView.setAdapter(firebaseRecyclerAdapter);
    }
