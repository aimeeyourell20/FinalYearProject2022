...
public class Goals_Adapter_Mentor extends RecyclerView.Adapter<Goals_Adapter_Mentor.GoalsViewHolder> implements Filterable {
    ...
    public Goals_Adapter_Mentor(List<Goals_Model> goals_models)
    {
    ...
    public Goals_Adapter_Mentor(Context c, ArrayList<Goals_Model> g){
        context = c;
        goals_models = g;
    }
    ...
    @NonNull
    @Override
    public Goals_Adapter_Mentor.GoalsViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View V = LayoutInflater.from(parent.getContext()).inflate(R.layout.goals_details, parent, false);

        return new Goals_Adapter_Mentor.GoalsViewHolder(V);
    }



    @Override
    public void onBindViewHolder(@NonNull Goals_Adapter_Mentor.GoalsViewHolder holder, int position) {
                ...
                builder.setItems(charSequence, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        if(i == 0){
                            ...
                            Goals.child(FirebaseAuth.getInstance().getUid()).child(goal_mentor.getGoalsmenteeid()).child(goals_model.getGoalsid()).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {

                                    if(task.isSuccessful()){
                                        Toast.makeText(context, "Goal Successfully Removed", Toast.LENGTH_SHORT).show();
                                        Intent intent = new Intent(context, Goals_Activity_Mentor.class);
